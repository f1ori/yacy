...
//                if (hasBody(res.getStatusCode())) {
                if (hasBody(client.getHttpResponse().getStatusLine().getStatusCode())) {
                    ...
                    OutputStream outStream = (gzippedOut != null) ? gzippedOut : ((chunkedOut != null)? chunkedOut : respond);
                    final Response response = new Response(
                            request,
                            requestHeader,
                            responseHeader,
        ...
        try {
            ...
            if (requestHeader.containsKey(RequestHeader.IF_MODIFIED_SINCE)) {
                ...
                if(AugmentedHtmlStream.supportsMime(cachedResponseHeader.mime())) {
                	outStream = new AugmentedHtmlStream(outStream, cachedResponseHeader.getCharSet());
                }
